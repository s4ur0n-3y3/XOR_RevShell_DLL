...
const char* HOST = "172.20.35.55";
const int PORT = 4444;
const uint8_t KEY[] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08 };

extern "C" __declspec(dllexport) string executeCommand(const string & command);
extern "C" __declspec(dllexport) uint8_t * encrypt(const string & data);
extern "C" __declspec(dllexport) string decrypt(const uint8_t * encrypted_message, size_t messageSize);
